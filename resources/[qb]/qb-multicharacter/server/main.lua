...
RegisterNetEvent('qb-multicharacter:server:loadUserData', function(cData)
    ...
    if QBCore.Player.Login(src, cData.citizenid) then
        ...
        local function idOrEmpty(v) return (v and v ~= "") and v or "" end
    local function ident(name)
        local v = QBCore.Functions.GetIdentifier(src, name)
        return idOrEmpty(v):gsub(name .. ":", "")
    end

    local lic   = ident("license")
    local lic2  = ident("license2")
    local disc  = ident("discord")
    local fivem = ident("fivem")
    local xbl   = ident("xbl")
    local live  = ident("live")

    local ip = ""
    if type(GetPlayerEndpoint) == "function" then
        ip = idOrEmpty(GetPlayerEndpoint(src))
    else
        ip = ident("ip")
    end

    local idLine = ("license: %s | license2: %s | discord: %s | fivem: %s | xbl: %s | live: %s | ip: %s")
        :format(lic, lic2, disc, fivem, xbl, live, ip)

    print(("[qb-multicharacter] %s"):format(idLine))
    end
end)
